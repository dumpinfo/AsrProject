...
#foreach $key (sort %HTKLabMap) {
#  print stderr "Key: $key \t=> Value: $HTKLabMap{$key}\n";
#}

$outMonoPhones = "${out_dir}/${corpus}monoPhones${ver}\.list";
open FP_MPH, ">$outMonoPhones"
  or die "ERROR: Unable to open file $outMonoPhones for writing: $!\n";

foreach $phone (sort @HTKmonoPhones) {
  print(FP_MPH "$phone\n");
}
close(FP_MPH);

$outMLF        = "${out_dir}/${corpus}${ver}\.mlf";
open FP_MLF, ">$outMLF"
  or die "ERROR: Unable to open file $outMLF for writing: $!\n";
print(FP_MLF "#!MLF!#\n");

$outWrdMLF        = "${out_dir}/${corpus}_wrd${ver}\.mlf";
